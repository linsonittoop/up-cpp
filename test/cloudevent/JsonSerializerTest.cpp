...
#include "up-cpp/cloudevent/serialize/JsonSerializer.h"
 ...
 */
TEST(JsonSerializerTest, SerializeInvalidCloudEvent) {
    CloudEvent cloudEvent = createBadCloudEvent();
    auto isValid = UCloudEventValidator::is_valid_event(cloudEvent);
    EXPECT_FALSE(isValid);
    std::vector<uint8_t> data = JsonSerializer().serialize(cloudEvent);
 ...
 */
TEST(JsonSerializerTest, SerializeEmptyCloudEvent) {
    CloudEvent emptyCloudEvent;
    std::vector<uint8_t> data = JsonSerializer().serialize(emptyCloudEvent);
    EXPECT_TRUE(data.size() == 0);
}
 ...
 */
TEST(JsonSerializerTest, DeserializeInvalidData) {
    auto serializer = JsonSerializer();
    CloudEvent cloudEvent = createValidCloudEvent();
    std::vector<uint8_t> data = serializer.serialize(cloudEvent);
    // TODO: Verify after implementation of serialize method.
    ...
    //TODO: Corrupt the serialized data
    // auto result = serializer.deserialize(data);
    // EXPECT_TRUE(result == std::nullopt);
}
 ...
 */
TEST(JsonSerializerTest, DeserializeEmptyData) {
    std::vector<uint8_t> data;
    auto result = JsonSerializer().deserialize(data);
    EXPECT_TRUE(result == std::nullopt);
}
